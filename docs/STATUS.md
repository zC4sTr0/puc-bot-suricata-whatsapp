...
**Verificado localmente nesta data** (execução real, não auto-relato): pytest `376 passed, 92 subtests`; unittest `374 OK`; `node --test` `39/39`; `compileall` e `shadow` exit 0; `--mode demo` determinístico em 2 execuções byte a byte; `--help` exit 0. **Não verificado:** CI no GitHub deste HEAD (o push desta onda ainda não aconteceu), build de imagem e qualquer estado de nuvem — os fatos de produção abaixo permanecem a referência mais recente por read-back.
