"""Caminho integrado offline da rodada.

O transporte HTTP é falso; o restante exercita o caminho completo:
``python -m suricata --mode rodada`` → ``CanvasClient`` → rodada →
armazenamento com CAS → relatório/exit code. As fixtures são sintéticas e não
representam uma execução de produção.

Não enfraqueça estas asserções para ficar verde.
"""
...
def _quiz_sem_prazo(agora: datetime) -> dict:
    ...
    return {
        "id": 100001, "name": "Quiz sintético", "points_possible": 3, "quiz_id": 555, "due_at": None,
        ...
        "html_url": "https://canvas.example.test/courses/100001/assignments/100001",
    }
...
class CaminhoRealTests(unittest.TestCase):
    ...
    def _transporte(self, method, url, headers, timeout):
        if "/assignments" in url:
            return self.status, {}, self.itens if "/courses/100001/" in url else []
        return 200, {}, [{"id": 100001, "name": "Curso sintético"}]
    ...
    def test_horario_do_aviso_em_brasilia(self):
        """O caminho integrado não depende de medições históricas."""
        from suricata.dominio.publico import Atividade, texto_novo

        a = Atividade("100001", "Curso sintético", "1", "Quiz", "quiz", None, None,
